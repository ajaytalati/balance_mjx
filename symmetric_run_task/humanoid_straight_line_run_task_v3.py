...
class HumanoidBalance(PipelineEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._ewa_alpha = alpha

        # Initialize EWA states for lower body joints (left and right)
        #self._ewa_left_ankle = 0.0
        #self._ewa_right_ankle = 0.0
        self._ewa_left_knee = 0.0
        self._ewa_right_knee = 0.0
        self._ewa_left_hip = 0.0
        self._ewa_right_hip = 0.0

    def _update_ewa(self, current, previous):
        """Calculate exponential weighted average."""
        return self._ewa_alpha * current + (1 - self._ewa_alpha) * previous
    ...
    def step(self, state: State, action: jp.ndarray) -> State:
        ...
        #left_ankle_qpos = data.qpos[self._left_ankle_joint]
        #right_ankle_qpos = data.qpos[self._right_ankle_joint]
        ...
        # Update EWA values
        self._ewa_left_knee = self._update_ewa(left_knee_qpos, self._ewa_left_knee)
        self._ewa_right_knee = self._update_ewa(right_knee_qpos, self._ewa_right_knee)
        self._ewa_left_hip = self._update_ewa(left_hip_qpos, self._ewa_left_hip)
        self._ewa_right_hip = self._update_ewa(right_hip_qpos, self._ewa_right_hip)
        
        # Calculate symmetry penalties
        # TODO ankle_symmetry = (self._ewa_left_ankle - self._ewa_right_ankle) ** 2
        knee_symmetry = (self._ewa_left_knee - self._ewa_right_knee) ** 2
        hip_symmetry = (self._ewa_left_hip - self._ewa_right_hip) ** 2
        
        ROM_symmetry_penalty = knee_symmetry + hip_symmetry #+ ankle_symmetry + knee_symmetry + hip_symmetry
        ...
        reward = (
            ...
            - ctrl_cost
            - 2 * ROM_symmetry_penalty # NEW: Penalize if joint on one side has a large ROM
            - 2 * asymmetry_penalty  # NEW: Penalize if one side is using more force
        ...
        state.metrics.update(
            ...
            ROM_symmetry_penalty=-ROM_symmetry_penalty
            )
    ...
    def reset(self, rng: jp.ndarray) -> State:
        ...
        metrics = {
            ...
            'reward_alive': healthy_reward,
            #TODO #ankle_symmetry=-ankle_symmetry,
            'knee_symmetry':-ctrl_cost,
            'hip_symmetry' : -ctrl_cost,
            'ROM_symmetry_penalty':-ctrl_cost,
        }
