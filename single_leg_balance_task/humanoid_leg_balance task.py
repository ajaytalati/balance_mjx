...
class HumanoidBalance(PipelineEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Load Mujoco Model (No changes here)
        ...
        sys = mjcf.load_model(mj_model)  # Use the correct loading function
        ...
        self._contact_penalty_weight = contact_penalty_weight
    ...
    def step(self, state: State, action: jp.ndarray) -> State:
        ...
        foot_pos = data.xpos[7]  # Approximate standing foot index
        stability_reward = jp.exp(-jp.linalg.norm(com[:2] - foot_pos[:2]))

        # ✅ Contact Penalty (If the **other** foot touches the ground)
        #contact_penalty = self._contact_penalty_weight * jp.any(data.contact[8:10])  # Penalize non-standing foot

        # ✅ Healthy Reward (Same as original)
    ...
    def reset(self, rng: jp.ndarray) -> State:
        ...
        # ✅ Compute total initial reward (REMOVED CONTACT PENALTY)
        reward = (
            self._pose_reward_weight * pose_reward
            + self._stability_reward_weight * stability_reward
            + healthy_reward
        ...
        # ✅ Updated metrics (Removed contact penalty)
        metrics = {
            'pose_reward': pose_reward,
            'stability_reward': stability_reward,
            'control_penalty': -ctrl_cost,
...
# Save as an MP4 file
video_path = "balance_task_rollout.mp4"
